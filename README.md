...
`Ctrl+K` searches all 61 tools. `Ctrl+Shift+S` grabs text off the screen from
...
> 实测数据；其余的软件里会明说，不假装能用。想自己构建见下方 [构建](#构建)。
...
![压到指定体积——56 KB 压到 9 KB，−84%](docs/screenshots/compress-target.png)

| 工具 | |
|---|---|
| **压到指定体积以内** | ✅ 二分搜索质量、逼近失败就降分辨率。网站限的是字节，TinyPNG 只给质量档。 |
| **抹除 EXIF 隐私** | ✅ 去掉 GPS 和设备信息，不重编码像素 |
| 批量压缩 · 格式转换 · 批量缩放 | ✅ 整个文件夹拖进来，递归处理 |
| **打码** | ✅ 直接重写像素，不是盖一层 |
| 加水印 | ✅ 含中文，平铺或单个 |
| **九宫格切图** | ✅ 先居中裁成方图，按发图顺序编号 |
| **长图拼接** | ✅ 宽度对齐到最窄那张，只缩不放 |
| 去白边 · 圆角与边框 · 按比例裁 | ✅ |
| 调色 · 生成 ICO · 主色调提取 · 转 Base64 | ✅ |
| 画布扩展 · GIF 拆帧与制作 | ✅ |
| 自动色阶 · 锐化 | ✅ |

### OCR —— 免费、离线、不订阅

iLovePDF、Smallpdf 把 OCR 锁在付费订阅里。Windows 自带的引擎比 Tesseract
更快更准，调它零成本、零体积。

![从发票图里提取文字，全程离线](docs/screenshots/ocr.png)

| 工具 | |
|---|---|
| 图片提取文字 | ✅ 单张 77 ms |
| 批量 OCR 合并导出 | ✅ |
| 截图取字 | ✅ 任何软件里按 `Ctrl+Shift+S`——唤起窗口、抓屏、框选一块 |
| **扫描件转可搜索** | ✅ 在 PDF 那栏。页面看着没变，文字铺在下面 |

### PDF

![页面整理——拖动重排、逐页旋转、勾掉不要的，再导出](docs/screenshots/pdf-pages.png)

| 工具 | |
|---|---|
| 合并 · 拆分 · 旋转 · 提取文字 · 图片转 PDF | ✅ |
| **扫描件转可搜索** | ✅ 隐形文字层盖在页面上——能搜、能选、能复制 |
| **压缩** | ✅ 一份 148 MB 的真实课件压掉 33% |
| **提取内嵌图片** | ✅ 按原样抠出，不重编码 |
| 反转页序 · 删除或保留指定页 | ✅ 支持 `1,3,5-8` 这样的区间 |
| **清除元数据** | ✅ /Info 和那份独立的 XMP 副本一起清 |
| 裁页边空白 | ✅ 只改裁剪框，可逆 |
| N 合 1 拼版 · 插入空白页 | ✅ |
| **修复损坏的 PDF** | ✅ 重建索引；实在不行降级成图片，并如实说明 |
| **PDF 转图片** | ✅ 用 Windows 渲染引擎，省掉 11 MB 的 pdfium DLL |
| **中文水印和页码** | ✅ 字体子集化：19.7 MB 字体 → 嵌入 13.5 KB |
| 解除限制 | ✅ 清掉打印/复制锁；打开密码仍需密码 |
| 设置密码 | ❌ 刻意不做——[见下](#刻意不做的) |
...
| 工具 | |
|---|---|
| **重复文件查找** | ✅ 按内容比对、不看名字。会保护程序依赖的文件。 |
| **批量重命名** | ✅ 规则可叠加、实时预览、留撤销日志 |
| **乱码修复** | ✅ 认出 GBK/Big5 转成 UTF-8，用的是 Firefox 的检测器 |
| **简繁转换** | ✅ 按词转换——头发→頭髮，但发展→發展 |
| **二维码** | ✅ 文件里一行生成一个；也能从图里读回来 |
| 查找替换 · 目录树导出 | ✅ 支持正则；编码自动检测 |
| 大文件分割与合并 | ✅ 字节级一致，验证过 |
| 按行去重排序 · CSV ↔ JSON · 批量改时间 | ✅ |
| **压缩成 ZIP** | ✅ 文件或文件夹打成一个包 |
| **解压、修复乱码名** | ✅ Windows 自己都搞坏的 GBK 条目名 |
| 按清单建文件夹 | ✅ |
| **文件粉碎** | ✅ 覆写后删除、独立面板、要手输确认——和普通删除彻底分开 |
| 文件哈希校验 | ✅ SHA-256 或 BLAKE3，分块读 |

## 实测，不是估算

**压到指定体积** —— 四张刻意挑的难压照片，WebP 输出：
...
第一版在最大那张上花了 44 s——每次探测都全分辨率重编码。改成按面积比一步估出初始
缩放后，降到 4.7 s。

**PDF 解析** —— 从一台在用的机器上翻出 1070 份真实 PDF：解析成功 1030 份（96.3%），
27536 页，共 10.1 s，最慢一份 759 ms。失败的 40 份全是真的坏文件。

**PDF 压缩** —— 12 份真实文档，148.5 MB → 99.4 MB（33%）。每份产物都重新打开、点过
页数：没有损坏，没有变大。

**重复文件查找** —— 8.9 分钟扫了 338190 个文件。抽样逐字节比对：零误判。
...
`Ctrl+K` 搜全部 61 个工具。`Ctrl+Shift+S` 在任何别的软件里都能唤起截图取字。
...
| | |
|---|---|
| 安装包 | **4.1 MB**（`.exe`）· 5.6 MB（`.msi`）|
| 装好后 | ~12 MB |
| 运行内存 | ~23 MB |

对比一下：Stirling-PDF 要跑 Docker，同等功能的 Electron 版起步就 150 MB 上下。
...
需要带 MSVC 工具链的 Rust、Node 22+，以及 Visual Studio C++ 生成工具。

## 授权

MIT。依赖审查见 [LICENSES-THIRD-PARTY.md](LICENSES-THIRD-PARTY.md)——
632 个依赖，无任何传染性许可证。PDF 压缩刻意没用 Ghostscript，因为它是 AGPL。
