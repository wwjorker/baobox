...
fn main() {
    ...
    let mut note = |ok: bool, label: &str, detail: String| {
        if ok {
            pass += 1;
            println!("  [OK]   {label:<26} {detail}")
        } else {
            fail += 1;
            println!("  [FAIL] {label:<26} {detail}")
        }
    };
    ...
    let r = std::panic::catch_unwind(|| image::open(&empty).map(|_| ()).map_err(|e| e.to_string()));
    note(
        matches!(r, Ok(Err(_))),
        "空文件",
        match &r {
    ...
    note(
        ...
        "超长中文路径",
        format!(
            "{} 字符，建目录 {deep_ok}，写文件 {long_file_ok}",
    ...
    image::DynamicImage::ImageRgba8(a)
        .write_to(&mut ab, image::ImageFormat::Png)
        .unwrap();
    std::fs::write(&alpha, ab.into_inner()).unwrap();
    let loaded = image::open(&alpha).unwrap();
    let r = baobox_lib::image_ops::compress_to_target(
        &loaded,
        baobox_lib::image_ops::OutFmt::WebP,
        50_000,
    );
    note(
        r.is_ok(),
    ...
    image::DynamicImage::ImageRgb8(t1)
        .write_to(&mut tb, image::ImageFormat::Png)
        .unwrap();
    std::fs::write(&tiny, tb.into_inner()).unwrap();
    let r = std::panic::catch_unwind(|| {
        let i = image::open(&tiny).unwrap();
        baobox_lib::image_ops::compress_to_target(&i, baobox_lib::image_ops::OutFmt::Jpeg, 1000)
            .is_ok()
    });
    note(
        matches!(r, Ok(true)),
    ...
    // ---- 8. 重命名：空规则链不应改动任何东西 ----
    let pv = baobox_lib::rename::rename_preview(vec![odd.to_string_lossy().to_string()], vec![]);
    note(
        pv.iter().all(|p| p.unchanged),
        "重命名空规则",
        format!(
            "{}/{} 标记为未变化",
            pv.iter().filter(|p| p.unchanged).count(),
            pv.len()
        ),
    );
